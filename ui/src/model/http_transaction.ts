...
export class HttpTransaction extends HttpTransactionBase {
  ...
  matchesMethodFilter(filter: WiretapFilters): Filter | boolean {
    if (
      filter?.filterMethod?.keyword?.toLowerCase() ===
      this.httpRequest?.method?.toLowerCase()
    ) {
      ...
      for (let i = 0; i < filter.filterKeywords.length; i++) {
        ...
        if (
          this.httpRequest.url
            .toLowerCase()
            .includes(keywordFilter.keyword.toLowerCase())
        ) {
        ...
        if (
          this.httpResponse?.responseBody
            ?.toLowerCase()
            .includes(keywordFilter.keyword.toLowerCase())
        ) {
          return keywordFilter;
        ...
        headers.forEach((value) => {
          if (
            value.toLowerCase().includes(keywordFilter.keyword.toLowerCase())
          ) {
            return keywordFilter;
          }
        });
...
export function BuildLiveTransactionFromState(
