...
export class WiretapControlsComponent extends LitElement {
  ...
  constructor() {
    ...
    this._filtersStore.subscribe(
      ...
      (filters: WiretapFilters) => {
        let numFilters = 0;
        if (filters?.filterKeywords.length > 1) {
          numFilters += filters.filterKeywords.length - 1;
        }
        filters.filterKeywords.forEach((filterKeyword: Filter) => {
          if (filterKeyword.id === SearchID) {
            this.searchInput.value = filters.filterKeywords[0].keyword;
          }
        });
  ...
  controlUpdateHandler(): BusCallback<CommandResponse> {
    return (msg: Message<CommandResponse<ControlsResponse>>) => {
      ...
      localforage.setItem<WiretapControls>(
        WiretapControlsStore,
        this._controls
      );
    };
  ...
  handleSearchChange() {
    ...
    this.filters.filterKeywords.forEach((filter: Filter) => {
  ...
  setInitialKeywordValue() {
    return this.filters.filterKeywords.find((filter: Filter) => {
      return filter.id === SearchID;
    }).keyword;
  }
  ...
  render() {
    ...
    if (this.numFilters > 0) {
      filtersBadge = html` <sl-badge class="filters-badge" pill pulse
        >${this.numFilters}</sl-badge
      >`;
    }
    ...
    return html`
      <div class="flex">
        <sl-input
          id="search-input"
          value="${this.setInitialKeywordValue()}"
      ...
      <sl-drawer
        label="wiretap controls"
        class="drawer-focus"
        id="controls-drawer"
      >
        <a id="downloadReport" style="display:none"></a>
        <wiretap-controls-settings
          globalDelay=${this._controls?.globalDelay}
          @globalDelayChanged=${this.handleGlobalDelayChange}
          @wipeData=${this.wipeData}
          @requestReport=${this.sendReportRequest}
        ></wiretap-controls-settings>
        <sl-button
