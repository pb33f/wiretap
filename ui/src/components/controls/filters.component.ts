...
export class WiretapControlsFiltersComponent extends LitElement {
  ...
  handleKeywordInput() {
    ...
    this.filters.filterKeywords.push({
      keyword: keyword,
  ...
  private removeKeyword(id: string) {
    // when pressing
    // when we remove search keyword
    // the search term becomes empty
    this.filters.filterKeywords = this.filters.filterKeywords.filter(
      (filter) => {
        if (id === SearchID && filter.id === SearchID) {
          filter.keyword = "";
          return true;
        }
  ...
  render() {
    ...
    const requestChainsFeature = html` <hr />
      <h3>Request Chains</h3>
      <p>Link related requests together using query parameter keys.</p>

      <sl-input
        @sl-change=${this.handleChainInput}
        class="label-on-left chain-input"
        type="text"
        label="Parameter"
    ...
    return html`
      <sl-select
        value="${this.filters?.filterMethod.keyword}"
        class="label-on-left"
        label="Method"
        @sl-change=${(change) => {
          this.methodFilterChanged(change.target.value);
        }}
        clearable
      >
        ${this._methods.map((method) => {
          return html` <sl-option value="${method}">
            <sl-tag variant="${ExchangeMethod(method)}" class="method"
              >${method}</sl-tag
            >
          </sl-option>`;
        })}
      ...
      <p>
        Keywords are matched against the request and response body and query
        parameters.
      </p>

      <sl-input
        @sl-change=${this.handleKeywordInput}
        class="label-on-left"
        type="text"
        label="Keyword"
        help-text="Press enter to add"
      ...
      <div class="keywords">
        ${this.filters?.filterKeywords?.map((filter) => {
          if (filter.id === SearchID && filter.keyword === "") {
            return html``;
          }
