...
export class HttpTransactionItemComponent extends LitElement {
  ...
  constructor(
    ...
    linkCache: TransactionLinkCache
  ) {
    super();
  ...
  render() {
    ...
    if (this._processing) {
      ...
      if (req && resp) {
        if (this._httpTransaction.httpResponse?.headers["Wiretap-Mock"]) {
          statusIcon = html`<p class="mocked-response">Ⓜ️</p>`;
        } else if (
          this._httpTransaction.requestValidation?.length > 0 ||
          this._httpTransaction.responseValidation?.length > 0
        ) {
          statusIcon = html`<sl-icon
    ...
    if (this._httpTransaction.containsChainLink) {
      ...
      chainLink = html`
        <sl-tooltip>
          <div slot="content">
            <strong>${total}</strong> parameter(s),
    ...
    return html` <div class="${tClass}" @click="${this.setActive}">
      <header>
      ...
      <div class="request-time">
        ${totalTime > 10000
          ? html`${(totalTime / 1000).toFixed(1)}s`
          : html`${totalTime > 0 ? totalTime : null}${totalTime > 0
              ? "ms"
              : null}`}
        <sl-icon name="arrow-left-right"></sl-icon>
      </div>
      <div class="transaction-status">
        ${this.hideControls ? "" : chainLink} ${statusIcon}
      </div>
    </div>`;
  }
}
